...
import { generateSecret, generateQRCode, verifyToken, generateBackupCodes } from '../utils/twoFactorAuth.js';
...
router.post('/2fa/setup', async (req, res) => {
  try {
    ...
    // Generate new secret
    const { secret, qrCode } = generateSecret(user.email);
    const qrCodeDataUrl = await generateQRCode(qrCode);
    const backupCodes = generateBackupCodes();

    // Store temporarily in session (user must verify with code before enabling)
    res.json({
      secret,
...
router.post('/2fa/verify', async (req, res) => {
  try {
    ...
    let isValid = verifyToken(user.twoFactorSecret, code);
